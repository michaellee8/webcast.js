(function() {
  ...
  if (typeof window !== "undefined") {
    ...
    AudioContext.prototype.createWebcastSource = function(bufferSize, channels, passThrough) {
      ...
      node.onaudioprocess = function(buf) {
        ...
        return (_ref2 = options.encoder) != null ? typeof _ref2.encode === "function" ? _ref2.encode(audio, function(data) {
          var _ref3;
          if (data != null) {
      ...
      node.connectSocket = function(encoder, url) {
        if (encoder instanceof Webcast.Recorder) {
          ...
          encoder.start(options.recorderSource.stream, function(data) {
            var _ref;
            if (data != null) {
              return (_ref = options.socket) != null ? _ref.sendData(data) : void 0;
            }
          });
        }
      ...
      node.close = function(cb) {
        var _ref;
        if ((_ref = options.recorderSource) != null) _ref.disconnect();
        options.recorderSource = null;
        return options.encoder.close(function(data) {
          var _ref2, _ref3;
          if (data != null) {
            if ((_ref2 = options.socket) != null) _ref2.sendData(data);
          }
          if ((_ref3 = options.socket) != null) _ref3.close();
  ...
  Webcast.Recorder = (function() {
    ...
    function Recorder(_arg) {
      this.channels = _arg.channels;
      this.info = {
        audio: {
          channels: this.channels,
    ...
    Recorder.prototype.start = function(stream, cb) {
      ...
      this.recorder.ondataavailable = function(e) {
        if (e.data.size > 0) return cb(e.data);
      };
      return this.recorder.start(100);
    };

    Recorder.prototype.close = function(cb) {
      this.recorder.stop();
      this.recorder = null;
      return cb();
    };
  ...
  Webcast.Socket = function(_arg) {
    ...
    socket.sendData = function(data) {
      ...
      if (data == null) return;
      if ((data.length != null) && (data.buffer != null)) {
